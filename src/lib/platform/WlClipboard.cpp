...
void WlClipboard::commitPendingData() const
{
  ...
  // wl-copy forks a long lived child to serve the selection, it must not
  // inherit our descriptors or it keeps sockets alive after we exit
  cmd.setUnixProcessParameters(QProcess::UnixProcessFlag::CloseFileDescriptors);
